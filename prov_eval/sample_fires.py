...
def init_bw_lineage_groupby(qid):
    con.execute("""CREATE TABLE lineage_{} AS SELECT (s.index+r.range_start) AS input, s.value AS output
                    ...
                    WHERE r.chunk_id=s.chunk_id ORDER BY s.value""".format(qid, qid, qid))
...
con.execute("CREATE TABLE fires AS SELECT * FROM read_csv_auto('/tmp/fires_with_dropped_cols.csv') LIMIT 100")

# enable lineage
con.execute("PRAGMA trace_lineage='ON'")
Q1 = "SELECT STAT_CAUSE_DESCR, COUNT(*) FROM fires GROUP BY STAT_CAUSE_DESCR"
#Q1 = "SELECT f1.FIRE_YEAR FROM fires f1 JOIN fires f2 ON f1.STATE = f2.STATE"
...
con.execute("PRAGMA profile_output='/tmp/profiled.json';")
...
res1 = con.execute("SELECT * from  HASH_GROUP_BY_{}_0_OUT".format(q1_id)).fetchdf()
with pd.option_context('display.max_rows', None, 'display.max_columns', None): 
    print(res1)
res1 = con.execute("SELECT * from  HASH_GROUP_BY_{}_0_SINK".format(q1_id)).fetchdf()
with pd.option_context('display.max_rows', None, 'display.max_columns', None): 
    print(res1)

init_bw_lineage_groupby(q1_id)

res1 = con.execute("SELECT * from  lineage_{}".format(q1_id)).fetchdf()
with pd.option_context('display.max_rows', None, 'display.max_columns', None): 
    print(res1)
...
tables_list = con.execute("PRAGMA show_tables").fetchdf()
print(tables_list)
#print("Selecting hash group by")
#res1 = con.execute("SELECT * from  HASH_GROUP_BY_5_0_SINK").fetchdf()
#print(res1)
#print("Scan result")
#res2 = con.execute("SELECT * from SEQ_SCAN_5_0_range").fetchdf()
#print(res2)

#print("Lineage result")
#res3 = con.execute("SELECT * from lineage_5").fetchdf()
#print(res3)
